...
public:
    GaussianErrorFunction(const std::vector<int32_t>& bins, const std::vector<uint64_t>& histogram)
        ...
        lsq::Matrixd&) override
    {
      ...
      for(Eigen::Index i = 0; i < bins.size(); ++i) {
        outVal[i] =  (state[0]*exp(-0.5*pow((this->bins[i]-state[1])/state[2], 2)) - histogram[i]);
      }
    }
private:
...
std::pair<double, double> calculateThresholds(std::vector<Block>& blocks,
    Image<double>& darkReference, int numberOfSamples, int backgroundThresholdNSigma,
    int xRayThresholdNSigma)
{
  ...
  int numberSamplePixels = detectorImageRows*detectorImageColumns*numberOfSamples;
  ...
  for(int i=0; i< numberOfSamples; i++) {
    ...
    std::uniform_int_distribution<int> randomFrameDist(0, randomBlock.header.imagesInBlock-1);
    ...
    for(int j=0; j<numberOfPixels; j++) {
      ...
      samples[i*numberOfPixels+j] = blockData[randomFrameIndex*numberOfPixels+j] - static_cast<int16_t>(darkReference.data[j]);
  ...
  auto maxBin = std::min(static_cast<int>(maxSample), static_cast<int>(mean+xrayThreshold*stdDev));
  auto minBin = std::max(static_cast<int>(minSample), static_cast<int>(mean-xrayThreshold*stdDev));
  ...
  GaussianErrorFunction* errorFunction = new GaussianErrorFunction(bins, histogram);
  optAlgo.setErrorFunction(errorFunction);
  lsq::Vectord initialState(3);
  auto indexOfMaxElement = std::max_element(histogram.begin(), histogram.end()) - histogram.begin();
  ...
  auto backgroundThreshold = result.state[1]+result.state[2]*backgroundThresholdNSigma;
...
// neighbors
std::vector<std::pair<int, int>> maximalPoints(const std::vector<uint16_t>& frame, int rows, int columns) {
...
std::vector<std::vector<std::pair<int, int>>> electronCount(std::vector<Block>& blocks, int scanRows, int scanColumns,
    Image<double>& darkReference, int numberOfSamples, int backgroundThresholdNSigma,
    int xRayThresholdNSigma)
{
  std::pair<double, double> thres = calculateThresholds(blocks, darkReference,
  ...
  for(const Block& block: blocks) {
    ...
    for(int i=0; i<block.header.imagesInBlock; i++) {
      ...
      std::vector<uint16_t> frame(frameStart, frameStart + block.header.rows*block.header.columns);
      ...
      events[block.header.imageNumbers[i]] = maximalPoints(frame, block.header.rows, block.header.columns);
    }
  }
