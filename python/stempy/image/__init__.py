...
from stempy.io import (
    ...
    SectorThreadedReader,
    SparseArray,
    get_hdf5_reader,
)


def create_stem_images(
    input,
    inner_radii,
    outer_radii,
    scan_dimensions=(0, 0),
    ...
    if isinstance(input, h5py._hl.files.File):
        ...
        imgs = _image.create_stem_images(input._electron_counted_data, inner_radii, outer_radii, center)
        ...
        if frame_dimensions is not None:
            ...
            if not scan_dimensions or scan_dimensions == (0, 0):
                msg = "scan_dimensions must be provided for np.ndarray " "raw data input"
                raise Exception(msg)
        ...
        raise Exception("Type of input, " + str(type(input)) + ", is not known to stempy.image.create_stem_images()")

    images = [np.array(img, copy=False) for img in imgs]
    return np.array(images, copy=False)


def create_stem_histogram(numBins, reader, inner_radii, outer_radii, scan_dimensions=(0, 0), center=(-1, -1)):
    ...
    """
    # create stem images
    imgs = _image.create_stem_images(reader.begin(), reader.end(), inner_radii, outer_radii, scan_dimensions, center)
...
def electron_count(
    ...
    if isinstance(reader, (SectorThreadedReader, SectorThreadedMultiPassReader)):
        ...
        if verbose:
            ...
            print("background threshold n sigma:", res.background_threshold_n_sigma)
...
def radial_sum(reader, center=(-1, -1), scan_dimensions=(0, 0)):
    ...
    sum = _image.radial_sum(reader.begin(), reader.end(), scan_dimensions, center)
...
def com_sparse(array, crop_to=None, init_center=None, replace_nans=True):
    ...
    :param init_center: optional; The initial center to use before cropping.
                            If this is not set then cropping will be applie around
                        the center of mass of the each full frame.
    :type init_center: tuple of ints of length 2
    :param replace_nans: If true (default) empty frames will have their center
                        of mass set as the mean of the center of mass
    ...
    for scan_position, frames in enumerate(array.data):
        ...
        if len(ev) > 0:
            ...
            com[:, scan_position] = (
                comy,
                comx,
            )  # save the comx and comy. Needs to be reversed (comy, comx)
        else:
...
def radial_sum_sparse(sa, center):
    ...
    for ii, frames in enumerate(sa.data):
        ...
        for _, events in enumerate(frames):
...
def plot_virtual_darkfield(image, centers_x, centers_y, radii, axes=None):
    ...
    This has the same center and radii inputs as stempy.image.virtual_darkfield
    so users can check their input is physically correct.
...
def mask_real_space(array, mask):
    ...
    :param mask: The mask to apply with 0 for probe positions to ignore and 1 for probe positions to include in the sum.
                Must have the same scan shape as array
